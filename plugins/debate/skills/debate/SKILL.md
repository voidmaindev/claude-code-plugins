...
9. **Judge the ledger** (you, the moderator — never the debaters):
   ...
   - Write `<conclusion>`: verdict and confidence; 2–4 sentences of reasoning; the decisive point IDs; a condition or mitigation for every con that is `conceded` or `stands`; the open questions the user must settle before committing (mostly the `contested` threads).
