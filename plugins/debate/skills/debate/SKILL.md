...
9. **Judge the ledger** (you, the moderator — never the debaters):
   ...
   - Compute the figures from the ledger: pros and cons standing (the sizes of the Pros/Cons lists) and raised in total; pros conceded by the Critic; cons conceded by the Advocate; contested and withdrawn counts; per side, the number of points in each final status (conceded / stands / narrowed / contested / withdrawn); rounds run of `<rounds>` with a stop label (`completed`, `stopped early: converged`, `stopped early: blocker accepted`, `stopped by user`); per round, objections raised, pros raised, concessions and withdrawals, plus the largest single-turn count for the chart scale; per turn, a one-line payload (IDs raised, IDs answered with which verb, what the moderator carried) for the sequence diagram; and per point, its thread as an ordered list of steps (raised → answer → reply → last word → final status, each with round and actor) for the track on its card.

10. **Render the report:**
    - Read the template. Drop its leading `<!-- … -->` documentation comment (the output starts at `<title>`), then replace every `{{PLACEHOLDER}}` exactly as that comment specifies — the verdict banner, the scoreboard numbers (plain integers; they also drive the outcome bars and the chart scale), the per-round bar chart columns, the sequence-diagram rows (one per turn), one card per argument with its thread track, and the transcript as speech cards: escape `<`, `>`, `&` in verbatim text; render the debaters' Markdown into the documented markup; give every point card `id="R1-C3"` / `id="R2-P1"` so the argument cards can link to it. Nothing outside the placeholders changes.
